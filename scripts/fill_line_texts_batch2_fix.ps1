# fill_line_texts_batch2_fix.ps1 - hex 17-32 (fix version)
...
foreach ($id in $texts.Keys) {
  ...
  # Match the exact line object containing this id
  $pattern = '("id":\s*"' + $escId + '"[\s\S]*?"text":\s*)"[^"]*"'
...
Write-Host "Batch 2 fix complete. Replaced $count text entries for hex 17-32."
