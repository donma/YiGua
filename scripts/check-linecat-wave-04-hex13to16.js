...
const ciW = lcArr.filter(x => x.hexagramId >= 13 && x.hexagramId <= 16);

const hexNames = ["同人","大有","謙","豫"];
const catNames = ["一般","工作事業","感情關係","財務金錢","人際合作","家庭親人","學習考試","身心狀態","重大決策","創業經營","官非合約","心境修行"];
const imagery = [
    "同人于門","無咎","同人于宗","吝","伏戎于莽","升其高陵","三歲不興","乘其墉","弗克攻","同人先號咷而後笑","大師克相遇","同人于郊","無悔","天與火","類族辨物","利涉大川","同人于野",
    "無交害","匪咎","艱則無咎","大車以載","有攸往","公用亨于天子","小人弗克","匪其彭","厥孚交如","威如","自天祐之","吉無不利","火在天上","遏惡揚善","順天休命",
    "謙謙君子","用涉大川","鳴謙","勞謙","有終吉","無不利","撝謙","不富以其鄰","利用侵伐","利用行師","征邑國","地中有山","裒多益寡","稱物平施",
    "鳴豫","介于石","不終日","盱豫","遲有悔","由豫","大有得","勿疑","朋盍簪","貞疾","恆不死","冥豫","成有渝","利建侯行師","雷出地奮","作樂崇德","殷薦之上帝"
];
...
console.log("=== WAVE 04 LINE CATEGORY HEX 13-16 ===");
...
const rbOk = ciW.filter(x=>x.reviewedBy==="line-category-gold-wave-04").length;
...
for (let h = 13; h <= 16; h++) { if (ciW.filter(x=>x.hexagramId===h).length !== 72) { console.log("Hex "+h+" count FAIL"); allPass=false; } }
...
let lineFail=0;for(let h=13;h<=16;h++)for(let l=1;l<=6;l++){const g=ciW.filter(x=>x.hexagramId===h&&x.line===l);const mu=new Set(g.map(x=>norm(x.meaning))).size,au=new Set(g.map(x=>norm(x.advice))).size,wu=new Set(g.map(x=>norm(x.warning))).size;if(mu!==12||au<11||wu<10){lineFail++;allPass=false;}}
...
for(let h=13;h<=16;h++)for(const c of allCats){const g=ciW.filter(x=>x.hexagramId===h&&x.category===c).sort((a,b)=>a.line-b.line);const mu=new Set(g.map(x=>norm(x.meaning))).size,au=new Set(g.map(x=>norm(x.advice))).size,wu=new Set(g.map(x=>norm(x.warning))).size;if(mu!==6||au<5||wu<5){catFail++;allPass=false;}}
...
console.log("\n=== WAVE 04 GATE: "+(allPass?"ALL PASS":"FAIL")+" ===");