...
const ciW = lcArr.filter(x => x.hexagramId >= 17 && x.hexagramId <= 20);

const hexNames = ["隨","蠱","臨","觀"];
const catNames = ["一般","工作事業","感情關係","財務金錢","人際合作","家庭親人","學習考試","身心狀態","重大決策","創業經營","官非合約","心境修行"];
const imagery = [
    "官有渝","出門交有功","係小子","失丈夫","係丈夫","失小子","隨有求得","利居貞","隨有獲","貞凶","有孚在道","以明","何咎","孚于嘉","拘係之","乃從維之","王用亨于西山","澤中有雷","嚮晦入宴息","元亨利貞",
...
console.log("=== WAVE 05 LINE CATEGORY HEX 17-20 ===");
...
const rbOk = ciW.filter(x=>x.reviewedBy==="line-category-gold-wave-05").length;
...
for (let h = 17; h <= 20; h++) { if (ciW.filter(x=>x.hexagramId===h).length !== 72) { console.log("Hex "+h+" count FAIL"); allPass=false; } }
...
let lineFail=0;for(let h=17;h<=20;h++)for(let l=1;l<=6;l++){const g=ciW.filter(x=>x.hexagramId===h&&x.line===l);const mu=new Set(g.map(x=>norm(x.meaning))).size,au=new Set(g.map(x=>norm(x.advice))).size,wu=new Set(g.map(x=>norm(x.warning))).size;if(mu!==12||au<11||wu<10){lineFail++;allPass=false;}}
...
for(let h=17;h<=20;h++)for(const c of allCats){const g=ciW.filter(x=>x.hexagramId===h&&x.category===c).sort((a,b)=>a.line-b.line);const mu=new Set(g.map(x=>norm(x.meaning))).size,au=new Set(g.map(x=>norm(x.advice))).size,wu=new Set(g.map(x=>norm(x.warning))).size;if(mu!==6||au<5||wu<5){catFail++;allPass=false;}}
...
console.log("\n=== FROZEN HASHES ===");
let fOk=true;
for (const [nm,fp,lo,hi] of [["Wave 01",frozen1Path,1,4],["Wave 02",frozen2Path,5,8],["Wave 03",frozen3Path,9,12],["Wave 04",frozen4Path,13,16]]) {
...
console.log("\n=== WAVE 05 GATE: "+(allPass?"ALL PASS":"FAIL")+" ===");