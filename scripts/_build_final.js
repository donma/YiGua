// Build final apply-wave05.js from all 4 hex data JSON files
const fs = require("fs");
const path = require("path");
const today = "2026-07-14";

// Load all data
const h17 = JSON.parse(fs.readFileSync(path.join(__dirname, "_w05_data_h17.json"), "utf8"));
const h18 = JSON.parse(fs.readFileSync(path.join(__dirname, "_w05_data_h18.json"), "utf8"));
const h19 = JSON.parse(fs.readFileSync(path.join(__dirname, "_w05_data_h19.json"), "utf8"));
const h20 = JSON.parse(fs.readFileSync(path.join(__dirname, "_w05_data_h20.json"), "utf8"));

const allData = [...h17, ...h18, ...h19, ...h20];
console.log("Total entries:", allData.length);
console.log("  H17:", h17.length, " H18:", h18.length, " H19:", h19.length, " H20:", h20.length);

if (allData.length !== 288) {
  console.error("Expected 288, got", allData.length);
  process.exit(1);
}

// Quick validation
for (const e of allData) {
  if (e[3].length < 90) { console.error("SHORT meaning:", e[0]+"-line-"+e[1]+"-"+e[2], e[3].length); process.exit(1); }
  if (e[4].length < 75) { console.error("SHORT advice:", e[0]+"-line-"+e[1]+"-"+e[2], e[4].length); process.exit(1); }
  if (e[5].length < 55) { console.error("SHORT warning:", e[0]+"-line-"+e[1]+"-"+e[2], e[5].length); process.exit(1); }
  if (!e[6] || e[6].length < 4) { console.error("SHORT basis:", e[0]+"-line-"+e[1]+"-"+e[2]); process.exit(1); }
}
...
out.push(`}`);
out.push(``);
out.push(`console.log("Applying ${allData.length} Wave 05 entries...");`);
out.push(`const wave05entries = [];`);
out.push(``);

// Generate entry calls (compact: pass arrays directly)
out.push(`const rawData = ${JSON.stringify(allData)};`);
out.push(`for (const e of rawData) {`);
out.push(`  wave05entries.push(E(e[0], e[1], e[2], e[3], e[4], e[5], e[6], e[7]));`);
out.push(`}`);
out.push(``);
