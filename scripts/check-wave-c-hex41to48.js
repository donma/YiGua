...
const ciW = catArr.filter(x => x.hexagramId >= 41 && x.hexagramId <= 48);
const rfW = reflArr.filter(x => x.hexagramId >= 41 && x.hexagramId <= 48);

const hexNames = ["損","益","夬","姤","萃","升","困","井"];
...
// ══════ WAVE A+B FROZEN ══════
console.log("=== WAVE A+B FROZEN CHECK ===");
const waCat = catArr.filter(x => x.hexagramId >= 25 && x.hexagramId <= 32);
const waRefl = reflArr.filter(x => x.hexagramId >= 25 && x.hexagramId <= 32);
const wbCat = catArr.filter(x => x.hexagramId >= 33 && x.hexagramId <= 40);
const wbRefl = reflArr.filter(x => x.hexagramId >= 33 && x.hexagramId <= 40);
const aOk = frozenA.cat === hashSection(waCat) && frozenA.refl === hashSection(waRefl);
const bOk = frozenB.cat === hashSection(wbCat) && frozenB.refl === hashSection(wbRefl);
console.log("Wave A: " + (aOk ? "UNCHANGED" : "CHANGED!"));
console.log("Wave B: " + (bOk ? "UNCHANGED" : "CHANGED!"));
if (!aOk || !bOk) { allPass = false; console.log("FROZEN WAVE CORRUPTED"); }

// ══════ CATEGORY ══════
console.log("\n=== WAVE C CATEGORY 41-48 ===");
...
console.log("gold=" + ciW.filter(x => x.qualityLevel === "gold").length + " reviewed=" + ciW.filter(x => x.reviewed === true).length);
...
console.log("reviewed="+rfW.filter(x=>x.reviewed===true).length+" qualityLevel_reviewed="+rfW.filter(x=>x.qualityLevel==="reviewed").length);
...
for(let h=41;h<=48;h++){const u=new Set(rfW.filter(x=>x.hexagramId===h).map(x=>norm(x.question))).size;console.log("hex "+h+": "+u+"/36 "+(u>=32?"PASS":"FAIL"));if(u<32)allPass=false;}
...
console.log("\n=== WAVE C GATE: " + (allPass ? "ALL PASS" : "FAIL") + " ===");
